...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(({ className, ...props }, ref) => (
	<div className="absolute   max-w-full w-full overflow-auto  rounded-md">
		<table ref={ref} className={cn("w-full border border-gray-200 caption-bottom text-sm mb-[100px]", className)} {...props} />
	</div>
));
Table.displayName = "Table";

const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(({ className, ...props }, ref) => <thead ref={ref} className={cn("relative bg-gray-100 max-w-full overflow-auto [&_tr]:border-b", className)} {...props} />);
TableHeader.displayName = "TableHeader";

const TableBody = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(({ className, ...props }, ref) => <tbody ref={ref} className={cn("relative max-w-full overflow-auto [&_tr:last-child]:border-0", className)} {...props} />);
